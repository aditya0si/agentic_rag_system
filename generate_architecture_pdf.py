    ...
    def draw_page_decorations(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(
                36,
                11 * inch - 25,
                "HCL Internship Technical Report — Agentic RAG Research Assistant",
            )
            self.drawRightString(
                8.5 * inch - 36,
                11 * inch - 25,
                "Author: Aditya Singh | GitHub: aditya0si/agentic_rag_system",
            )
...
def create_langgraph_flowchart_drawing():
    ...
    for label, bg_hex, x in nodes:
        d.add(
            Rect(
                ...
                rx=5,
                ry=5,
                fillColor=colors.HexColor(bg_hex),
        ...
        if x < 465:
            ...
            d.add(
                Polygon(
                    [arrow_end_x, 47, arrow_end_x - 4, 50, arrow_end_x - 4, 44],
    ...
    d.add(
        String(
            270,
            6,
            "Figure 1: State transitions in the 5-Agent LangGraph State Machine",
...
def create_ingestion_flowchart_drawing():
    ...
    for label, bg_hex, x in steps:
        ...
        if x < 455:
            d.add(
                Line(
                    x + 75, 39, x + 98, 39, strokeColor=colors.HexColor("#64748B"), strokeWidth=1.5
                )
            )
            d.add(
                Polygon(
                    [x + 98, 39, x + 94, 42, x + 94, 36],
                    fillColor=colors.HexColor("#64748B"),
                    strokeColor=colors.HexColor("#64748B"),
                )
            )
    ...
    d.add(
        String(
            270,
            4,
            "Figure 2: Document Ingestion, Token Chunking, and Embedding Storage Pipeline",
            fontName="Helvetica-Oblique",
            fontSize=8.5,
            textAnchor="middle",
            fillColor=colors.HexColor("#475569"),
...
def generate_pdf(output_filename="rag_architecture_HCL_internship.pdf"):
    ...
    header_data = [
        [
            ...
            Paragraph(
                f"<b>HCL INTERNSHIP PORTFOLIO</b><br/><a href='{repo_url}'><u>github.com/aditya0si/agentic_rag_system</u></a>",
                meta_style,
            ),
        ],
        [
            Paragraph(
                "Production-Grade Multi-Agent RAG System with LangGraph, Cross-Encoder Reranking & Hallucination Guardrails",
                subtitle_style,
            ),
    ...
    diff_data = [
        [
            Paragraph("Architectural Feature", table_header_style),
            Paragraph("Naive RAG Baseline", table_header_style),
            Paragraph("Agentic RAG Implementation", table_header_style),
            Paragraph("Impact / Benefit", table_header_style),
        ],
        [
            Paragraph("State Orchestration", table_cell_bold),
            Paragraph("Linear single-pass chain", table_cell_style),
            Paragraph("5-Node LangGraph State Machine", table_cell_style),
            Paragraph("Dynamic routing, state trace & node retries", table_cell_style),
        ],
        [
            Paragraph("Retrieval Precision", table_cell_bold),
            Paragraph("Top-K vector cosine distance", table_cell_style),
            Paragraph("Bi-Encoder + Cross-Encoder Re-ranker", table_cell_style),
            Paragraph("Re-ranks ms-marco models for semantic precision", table_cell_style),
        ],
        [
            Paragraph("Context Filtering", table_cell_bold),
            Paragraph("Raw chunks passed directly", table_cell_style),
            Paragraph("LLM Binary Relevance Grader", table_cell_style),
            Paragraph("Filters out irrelevant noise prior to synthesis", table_cell_style),
        ],
        [
            Paragraph("Hallucination Defense", table_cell_bold),
            Paragraph("None (relies on system prompt)", table_cell_style),
            Paragraph("Independent Hallucination Checker", table_cell_style),
            Paragraph("Cross-verifies generated claims vs source context", table_cell_style),
        ],
        [
            Paragraph("Observability", table_cell_bold),
            Paragraph("Basic console print statements", table_cell_style),
            Paragraph("structlog + Prometheus + SSE Stream", table_cell_style),
            Paragraph("Per-node latency tracking & request correlation", table_cell_style),
        ],
    ]
    ...
    if os.path.exists("ui_screenshot_chat.png"):
        story.append(
            Paragraph(
                "<b>UI Screenshot 1: Interactive Chat Interface & Grounded Citations</b>", h2_style
            )
        )
    ...
    if os.path.exists("ui_screenshot_metrics.png"):
        story.append(
            Paragraph(
                "<b>UI Screenshot 2: Real-time Telemetry & RAGAS Benchmark Dashboard</b>", h2_style
            )
        )
    ...
    agents_info = [
        (
            ...
            "backend/core/agents/query_rewriter.py",
        ...
        (
            ...
            "backend/core/agents/retriever.py & reranker.py",
        ...
        (
            ...
            "backend/core/agents/relevance_grader.py",
        ...
        (
            ...
            "backend/core/agents/answer_generator.py",
        ...
        (
            "5. Hallucination Checker Agent",
            "backend/core/agents/hallucination_checker.py",
    ...
    tech_data = [
        [
            ...
            Paragraph("Rationale / Key Features", table_header_style),
        ],
        [
            Paragraph("Backend Framework", table_cell_bold),
            Paragraph("FastAPI + Uvicorn", table_cell_style),
            Paragraph(
                "Asynchronous execution, auto OpenAPI docs, Pydantic data validation",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Agent Orchestration", table_cell_bold),
            Paragraph("LangGraph 0.2+", table_cell_style),
            Paragraph(
                "Explicit cyclic state machine, state persistence, debuggable node traces",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Vector Database", table_cell_bold),
            Paragraph("ChromaDB (Local)", table_cell_style),
            Paragraph(
                "Zero-cost, embedded persistence, metadata filtering by document ID",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Embeddings", table_cell_bold),
            Paragraph("all-MiniLM-L6-v2", table_cell_style),
            Paragraph(
                "Free local inference, 384-dim semantic embeddings with low latency",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Re-ranking Engine", table_cell_bold),
            Paragraph("ms-marco-MiniLM-L-6-v2", table_cell_style),
            Paragraph(
                "Cross-encoder model improving retrieval precision over bi-encoders",
                table_cell_style,
            ),
        ],
        [
            Paragraph("LLM Provider", table_cell_bold),
            Paragraph("Google Gemini 1.5 / OpenAI", table_cell_style),
            Paragraph(
                "High token window, fast response times, cost-effective structured output",
                table_cell_style,
            ),
        ],
        [
            ...
            Paragraph(
                "Rapid reactive web interface with native chat components & SSE streaming",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Observability", table_cell_bold),
            Paragraph("structlog + Prometheus", table_cell_style),
            Paragraph(
                "JSON formatted logs, X-Request-ID request tracing, custom metrics",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Security", table_cell_bold),
            Paragraph("Token-Bucket Rate Limiter", table_cell_style),
            Paragraph(
                "Defensive rate limiting, prompt injection detection, input sanitization",
                table_cell_style,
            ),
        ],
        [
            Paragraph("Evaluation", table_cell_bold),
            Paragraph("RAGAS Framework", table_cell_style),
            Paragraph(
                "Standardized evaluation for faithfulness, relevancy, precision, recall",
                table_cell_style,
            ),
        ],
    ]
    ...
    sec_table = Table(
        [[Paragraph(sec_title, h2_style)], [Paragraph(sec_text, body_style)]], colWidths=[540]
    )
    sec_table.setStyle(
        TableStyle(
            [
    ...
    file_index_data = [
        [
            ...
            Paragraph("Repository File Path", table_header_style),
            Paragraph("Key Symbols / Classes", table_header_style),
        ],
        [
            Paragraph("FastAPI App Entry", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/main.py'><u>backend/main.py</u></a>",
                code_path_style,
            ),
            Paragraph("app, lifespan, routers", table_cell_style),
        ],
        [
            Paragraph("LangGraph Pipeline", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/core/agents/graph.py'><u>backend/core/agents/graph.py</u></a>",
                code_path_style,
            ),
            Paragraph("AgentState, build_graph()", table_cell_style),
        ],
        [
            Paragraph("Query Rewriter", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/core/agents/query_rewriter.py'><u>backend/core/agents/query_rewriter.py</u></a>",
                code_path_style,
            ),
            Paragraph("rewrite_query()", table_cell_style),
        ],
        [
            Paragraph("Vector Store Manager", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/core/vector_store.py'><u>backend/core/vector_store.py</u></a>",
                code_path_style,
            ),
            Paragraph("VectorStoreManager", table_cell_style),
        ],
        [
            Paragraph("Cross-Encoder Reranker", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/core/agents/reranker.py'><u>backend/core/agents/reranker.py</u></a>",
                code_path_style,
            ),
            Paragraph("rerank_chunks()", table_cell_style),
        ],
        [
            Paragraph("Hallucination Checker", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/backend/core/agents/hallucination_checker.py'><u>backend/core/agents/hallucination_checker.py</u></a>",
                code_path_style,
            ),
            Paragraph("check_hallucination()", table_cell_style),
        ],
        [
            Paragraph("Streamlit UI Application", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/frontend/app.py'><u>frontend/app.py</u></a>",
                code_path_style,
            ),
            Paragraph("main(), render_chat()", table_cell_style),
        ],
        [
            Paragraph("RAGAS Evaluation Framework", table_cell_bold),
            Paragraph(
                f"<a href='{repo_url}/tree/main/eval/run_evaluation.py'><u>eval/run_evaluation.py</u></a>",
                code_path_style,
            ),
            Paragraph("evaluate_ragas()", table_cell_style),
        ],
    ]
