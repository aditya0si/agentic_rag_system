...
def get_embeddings() -> Embeddings:
    ...
    if EMBEDDING_PROVIDER == "openai":
        return OpenAIEmbeddings(model=OPENAI_EMBEDDING_MODEL, api_key=SecretStr(OPENAI_API_KEY))
...
class VectorStoreManager:
    ...
    def query(
        self, query_text: str, doc_ids: list[str] | None = None, top_k: int | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        results = vectorstore.similarity_search_with_score(
            query=query_text, k=top_k, filter=search_filter
        )
