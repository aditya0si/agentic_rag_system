...
if not has_key:
    ...
    print(
        f"API key detected for provider: {config.LLM_PROVIDER}. Running actual agentic pipeline..."
    )
...
def main():
    ...
    try:
        ...
        assert result.get("agent_trace") == expected_trace, (
            f"Expected trace {expected_trace}, got {result.get('agent_trace')}"
        )
