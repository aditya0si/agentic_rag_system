...
def create_sample_files():
    ...
    c.drawString(
        100, 680, "The Hallucination Checker validates generated answers against retrieved sources."
    )
    c.showPage()
...
def main():
    ...
    try:
        ...
        for idx, chunk in enumerate(txt_chunks):
            print(
                f"Chunk {idx}: {chunk['chunk_id']} (Page {chunk['page_number']}): {chunk['chunk_text'][:60]}..."
            )
