...
if not st.session_state.messages:
    ...
    for column, value, label in zip(
        ...
        (
            "Choose exactly which documents to search",
            "Inspect the original passage and page",
            "Retrieval, relevance checks, and grounding",
        ),
        strict=True,
    ):
        with column:
    ...
    if st.session_state.uploaded_docs:
        ...
        for column, prompt in zip(starter_cols, starters, strict=True):
            with column:
                if st.button(
                    prompt, key=f"starter_{starters.index(prompt)}", use_container_width=True
                ):
...
question = st.chat_input("Ask a question about your uploaded documents...", disabled=disabled)
...
if question:
    ...
    # Guard: backend down
        ...
        with st.chat_message("assistant"):
            status_container = st.status("🧠 **Agentic RAG pipeline running...**", expanded=True)

            with status_container:
                st.write("🔍 Rewriting query to resolve conversational context...")
                selected_docs = st.session_state.get("selected_doc_ids", None)
                response = ask_question_api(question, st.session_state.session_id, selected_docs)
